...
namespace CarRendalSystem.Bll.Validators
{
    ...
    {
        public CarGetDtoValidator(ICarRepo carRepo)
        {
            ...
            // Bookings kolleksiyasi bo‘sh bo‘lmasligi yoki null bo‘lmasligi kerak
            RuleFor(car => car.Bookings)
                .NotNull().WithMessage("Bookings bo‘sh bo‘lishi mumkin emas.");

            // Reviews kolleksiyasi null bo‘lmasligi kerak
            RuleFor(car => car.Reviews)
                .NotNull().WithMessage("Reviews bo‘sh bo‘lishi mumkin emas.");
        }
    }
}
